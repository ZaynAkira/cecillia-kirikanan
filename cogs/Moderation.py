...
class Moderasi(commands.Cog):
    ...
    async def ban(self, ctx:Context, member:Member=None, *, alasan:str=None):
        if not member:
            return await ctx.send("Please write down who do you want to ban with the reason, senpai! \nExample: `c.ban @Kirikanan#4821 Ban Hammer has Spoken`")

        alasan = "No Particular Reason" if not alasan else alasan
        ...
        if alasan is None:
            await member.send(f"I'm sorry, but you already been banned from **{ctx.guild.name}**.")
        else:
            await member.send(f"I'm sorry, but you already been banned from **{ctx.guild.name}** dengan alasan **{alasan}**.")
    @ban.error
    ...
    async def purge(self, ctx:Context, jumlah:int=None):
        if not jumlah:
            return await ctx.send("Please write down how many messages did you want to purge.\nExample: `c.purge 3`")

        await ctx.channel.purge(limit=jumlah+1)
        await ctx.send(f"Successfully purged {jumlah} messages.", delete_after=2.5)
    @purge.error
    async def on_purge_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.reply("Sorry Senpai, Senpai doesn't have permission to do this command...")
        elif isinstance(error, commands.BotMissingPermissions):
            await ctx.reply("Cecillia don't have any permission to delete messages, Sorry Senpai...")
...
async def setup(client):
    await client.add_cog(Moderasi(client))
