...
class Profil(commands.Cog):
    ...
    async def avatar(self, ctx, *, member: discord.Member=None):
        ...
        async with ctx.typing():
            ...
            #bikin embed
            embed = discord.Embed(
                title = f'Here is {member.name}❜s profile picture:',
    ...
    async def serverinfo(self, ctx):
        ...
        try:
            ...
            embed.add_field(name="Created since", value=f"{tanggal}/{bulan}/{tahun} {jam}:{menit}:{detik}")
        except:
            embed.add_field(name="Created since", value="(Failed to gain information)")
        ...
        try:
            embed.add_field(name="Total Text Channel", value=len(ctx.guild.text_channels))
            embed.add_field(name="Total Voice Channel", value=len(ctx.guild.voice_channels))
            embed.add_field(name="Total Category", value=len(ctx.guild.categories))
        except:
            embed.add_field(name="Total Text Channel", value="[Failed to load Info]")
            embed.add_field(name="Total Voice Channel", value="[Failed to load Info]")
            embed.add_field(name="Total Category", value="[Failed to load Info]")

        try:
            if ctx.guild.region is not None:
                embed.add_field(name="Region", value=str(ctx.guild.region).capitalize())
            else:
                embed.add_field(name="Region", value="Nothing")
        except:
            embed.add_field(name="Region", value="[Failed to load Info]")

        if ctx.guild.shard_id is not None:
            embed.add_field(name="ID Shard", value=ctx.guild.shard_id)
        else:
            embed.add_field(name="ID Shard", value="Nothing")
        if ctx.guild.afk_channel is not None:
            embed.add_field(name="AFK Channel", value=len(ctx.guild.afk_channel.mention))
        else:
            embed.add_field(name="AFK Channel", value="Nothing")

        if ctx.guild.owner is not None:
            embed.add_field(name="Server❜s Owner", value=f"{ctx.guild.owner.mention} (`{ctx.guild.owner}`)")
        else:
            embed.add_field(name="Server❜s Owner", value="Cannot find Server❜s Owner")
        embed.add_field(name="Boost Level", value=f"Level {ctx.guild.premium_tier} ({ctx.guild.premium_subscription_count} Boost)")
...
async def setup(client):
    await client.add_cog(Profil(client))
