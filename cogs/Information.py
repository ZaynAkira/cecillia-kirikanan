...
class Information(commands.Cog):
    ...
    async def on_enlarge_error(self, ctx, error):
        if isinstance(error, commands.CommandError):
            await ctx.reply("Unfortunately, bot can't enlarge this emote somehow")
    ...
    async def kbbi(self, ctx, *kata):
        try:
            if not kata:
                await ctx.reply("**Sintaks tidak valid!** Masukkan kata yang ingin kamu lihat di KBBI!\nContoh: `r!kbbi bantal`")
            else:
                async with ctx.typing():
                    khata = "%20".join(kata)
    ...
    async def roleinfo(self, ctx:commands.Context, *, role:discord.Role=None):
        if role is None:
            return await ctx.send("**Invalid Syntax**! Please write a role that you want to see their information!\nExample: `r!roleinfo [role]`")
        ...
        if role.hoist is True:
            embed.add_field(name="Separated?", value="✅")
        else:
            embed.add_field(name="Separated?", value="❌")
        #
        if role.mentionable is True:
            embed.add_field(name="Mention-able?", value="✅")
        else:
            embed.add_field(name="Mention-able?", value="❌")
        #
        if role.managed is True:
            embed.add_field(name="Manage-able?", value="✅")
        else:
            embed.add_field(name="Manage-able?", value="❌")

        if role.is_default():
            embed.add_field(name="Default Role?", value="✅")
        else:
            embed.add_field(name="Default Role?", value="❌")
        #
        ...
        if role.is_bot_managed():
            embed.add_field(name="Set from bot?", value="✅")
        else:
            embed.add_field(name="Set from bot?", value="❌")

        embed.add_field(name="Position", value=role.position)
        embed.add_field(name="Created since", value=role.created_at.strftime("%d/%m/%Y %H:%M:%S UTC"))
        embed.add_field(name="Total Member that have", value=len(role.members))

        if sum(len(izin) for izin in [perm[0] for perm in role.permissions if perm[1]]) < 1024:
            embed.add_field(name=f"Permissions ({len([perm[0] for perm in role.permissions if perm[1]])})", value=f'`{", ".join([perm[0] for perm in role.permissions if perm[1]]).replace("_", " ")}`', inline=False)
        else:
            embed.add_field(name=f"Permissions ({len([perm[0] for perm in role.permissions if perm[1]])})", value='does not fit', inline=False)
